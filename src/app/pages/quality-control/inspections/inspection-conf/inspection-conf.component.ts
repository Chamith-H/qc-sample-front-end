...
export class InspectionConfComponent {
  ...
  beginInspection(s_data: any) {
    ...
    const body = {
      ...
      samplingMethod: s_data.samplingMethod,
      samplingLogics: s_data.samplingLogics,
    };
  ...
  ngOnInit() {
    if (this.data.U_Approval === "Open") {
      ...
      this.inspectionService.startConf(body).subscribe({
        next: (data: any) => {
          if (data) {
            this.samplingMethodology.method = data.method;
            this.samplingMethodology.samplingMethod = data.samplingMethod;

            const neededSamplingLogics = data.samplingLogics.filter(
              (s_logic: any) => s_logic.uom === requestData.Uom
            );

            this.samplingMethodology.samplingLogics = neededSamplingLogics;
          }
